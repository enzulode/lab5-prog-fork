...
public class DataClassesValidator
{
	...
	public void validateCoordinatesXCoord(float x) throws ValidationException
	{
		...
		if (x == Float.POSITIVE_INFINITY) throw new OutOfAllowedBoundException("X coordinate cannot be infinity.");
	}
	...
	{
		if (creationDate == null) throw new FieldCannotBeNullException("Ticket creation date cannot be null.");
	}

	public void validateTicketPrice(float price) throws ValidationException
	{
		if (price <= 0) throw new OutOfAllowedBoundException("Ticket price should be greater than zero.");
	}
	...
	public void validateTicketComment(String comment) throws ValidationException
	{
		...
		if (comment.length() > 404) throw new OutOfAllowedBoundException("Ticket comment length cannot be more than 404 chars long.");
	}

	public void validateTicketRefundable(Boolean refundable) throws ValidationException
	{
		if (refundable == null) throw new FieldCannotBeNullException("Ticket refundable status cannot be null.");
	}
