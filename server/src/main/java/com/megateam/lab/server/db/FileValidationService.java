...
public class FileValidationService
{
	...
	public void validateSavingFilePermissions(@NonNull File file) throws DatabaseException
	{
		if (!Files.isReadable(Path.of(file.toURI())))
			throw new DatabaseFilePermissionException("Database file is not readable. Change the permissions.");

		if (!Files.isWritable(Path.of(file.toURI())))
			throw new DatabaseFilePermissionException("Database file is not writable. Change the permissions.");
	}
}
